...
- 计划Scene：`Assets/Scenes/Levels/Snow/Snow_009.unity`
...
- 主要目标：用两道门分段安排敌人进入寒冰
- 多阶段内容：
   - 阶段1：使用Plate-1打开Door-1，把Enemy-A放入中段。
   - 阶段2：调整Enemy-A朝向并使用Plate-2打开Door-2；同时处理反向巡逻的Enemy-B。
   - 阶段3：利用正确冻结位置登上右侧上层分支出口。
- 使用机制：双压力板、双门、可冻结敌人、FrozenGround
