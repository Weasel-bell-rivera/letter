...
- 开放灰盒条件：
  ...
  - 已完成：现有`PressurePlate2D.prefab`旋转为墙面实例后的占用、视觉和重置经过验证。
...
- 房间名称：镜门寒阶。
- 房间类型：雪区安全组合房。
- 主要目标：让MirrorClone进入Player无法到达的竖直控制井并踩住压力板，打开敌人门，使巡逻敌人走上寒冰地面冻结；Player再把冻结敌人作为踏板到达返回出口。
- 预期洞察：镜像不仅能代替Player维持远处机关，还能把活动危险转化为可利用的地形。
- 操作压力：低。
- 预计首次完成时间：约`60～120秒`。
- 本房没有限时解冻、随机敌人路线、移动平台、坠落伤害或强制精确跳跃。

由于本房使用特殊镜墙，只有在玩家已经理解特殊墙面镜的输入旋转后，才应把本房安排为必经教学内容。世界访问顺序未批准前，本房保持独立可读，但不声明自己是特殊镜墙的首次教学房。
...
│             │                                        │
│  Special    │                                        │
│  MirrorWall R   P ─────────── Observation ─────┐     │
│            ║====================================│     │
│            ║                                    v     │
│ Entrance-A ║   Enemy-A →  |Door-A|  r  III F  ####   │
│     ↑      ║====================================####   │
│ SNOW_002   ║                              return path │
│                                               ↓      │
│                                           Exit-A     │
│                                           SNOW_002   │
└──────────────────────────────────────────────────────┘
...
- `R`：特殊墙面镜的预期放置点。
- `C`：MirrorClone在竖直控制井中的预期路线。
- `[Plate-A]`：旋转安装在MirrorClone局部地面上的压力板。
...
- Player从入口可以直接看到特殊镜墙、竖直控制井、`Plate-A`、`Door-A`、Enemy-A、寒冰地面和目标高台。
- 特殊镜墙位于Player左侧。Player面向左并紧邻墙面放置镜子。
- 按已确定的墙镜映射，墙在Player左侧时，Player向右输入使MirrorClone向上移动。
- Player沿上层观察通道向右移动时，MirrorClone沿控制井向上移动到`Plate-A`。

### 竖直控制井

- 控制井与Player路线通过实体地形完全分隔。
- Player不能使用标准跳跃、边缘站立或Collider缝隙进入控制井。
- MirrorClone可以按特殊墙镜的统一旋转输入和局部重力规则稳定到达`Plate-A`。
- `Plate-A`使用现有`PressurePlate2D.prefab`旋转安装，表面朝向与MirrorClone局部重力相反，使MirrorClone停止输入后仍能稳定维持占用。
- 旋转只改变实例Transform和表现方向，不改变压力板的触发资格、占用、反馈或重置规则。
...
- `r`区通过静态台阶提供返回观察通道和镜墙放置点的路线，提前回收镜子不会造成软锁。
...
| `Terrain` | 入口、观察通道、控制井边界、Enemy-A普通巡逻地面、`r`安全区、高台和返回通道 | 实体碰撞，`StaticSolid` |
| `FrozenGround` | `Door-A`右侧、目标高台前的短寒冰地面 | 实体碰撞，`FrozenGround` |
| `SpecialMirrorWall` | Player左侧的特殊垂直镜墙 | 实体碰撞，`SpecialMirrorWall` |
| `Decoration` | 不参与玩法的积雪、冰晶和环境装饰 | 无碰撞 |
| `Foreground` | 不遮挡关键机关的少量前景 | 无碰撞 |

- 本房不使用`OneWayPlatform`或`Hazard` Tilemap。
- 普通Terrain、寒冰地面、特殊镜墙和装饰冰雪必须具有清晰不同的视觉语言。
...
- MirrorClone离开、死亡或被回收后，`Plate-A`释放，`Door-A`按防夹规则关闭。
- Enemy-A一旦在门右侧冻结，即使镜像回收、压力板释放和门关闭，也保持冻结直到本次房间尝试重置。
- 敌人不能触发`Plate-A`。
...
|---|---|---|---|
| `Plate-A` | `PressurePlate2D` | `Assets/Prefabs/Gameplay/Switches/PressurePlate2D.prefab` | 已存在并通过90度旋转实例测试；允许Player和MirrorClone，但空间上只有MirrorClone可达 |
...
1. Player观察控制井中的`Plate-A`与下方`Door-A`，通过开关和门的状态变化理解控制关系；两者之间不绘制连线。
2. Player面向左并紧邻特殊镜墙放置镜子。
3. MirrorClone生成在墙镜规则确定的位置和局部物理空间中。
4. Player向右移动到观察区；同一输入使MirrorClone沿控制井向上移动。
5. MirrorClone到达并稳定占用`Plate-A`，`Door-A`开启。
6. Enemy-A继续固定巡逻，最终向右通过`Door-A`。
7. Enemy-A跨过门右侧普通地面，脚部首次稳定接触`FrozenGround`并进入`Frozen`状态。
8. Player确认冻结完成后右键回收镜子；`Plate-A`释放，`Door-A`关闭，Enemy-A保持冻结。
9. Player从观察区下降到`r`安全区。
10. Player踩上冻结的Enemy-A，再跳上目标高台。
11. Player沿静态返回通道下降，从`Exit-A`返回`SNOW_002`。

## 失败、重置与重复尝试

### 提前回收镜子

- MirrorClone立即消失并释放`Plate-A`。
- `Door-A`尝试关闭；若敌人在门路径中则等待路径清空。
- Enemy-A尚未冻结时继续保持活动状态和当前巡逻阶段。
- Player可以通过`r`区返回控制区并重新放置镜子，不需要死亡或强制重置。

### MirrorClone接触失败

- 控制井不包含敌人或危险物，MirrorClone不应在预期路线中死亡。
- 如果因非法碰撞或越界死亡，镜子按统一规则回手，Enemy-A和普通门状态不进行整体重置。
...
- 静态Terrain、`FrozenGround`和`SpecialMirrorWall` Tilemap保持不变。
...
- Player不能进入控制井或直接触发`Plate-A`，因此镜像是打开`Door-A`的必要手段。
...
- 前景和装饰不得遮挡控制井、门、敌人、寒冰边界或出口。

## 验收标准

- Scene编号、路径以及到`SNOW_002`的连接与地图一致。
- 只有MirrorClone能通过房间空间到达`Plate-A`。
- 特殊墙镜输入映射完全遵循通用镜子规则，没有房间专用方向配置。
- MirrorClone稳定占用`Plate-A`时`Door-A`保持开启，离开或回收后释放。
...
- Player无法不使用镜像直接打开敌人路线，也无法绕过冻结踏板到达出口。
- 提前回收镜子、MirrorClone消失、Player死亡、手动重置和重新进房均不会留下错误门、敌人或占用状态。
