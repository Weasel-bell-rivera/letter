...
- Unity Scene：`Assets/Scenes/Levels/Earth/Earth_010.unity`（已创建并登记Build Settings）。
- 地图连接：左`EARTH_006`，下`EARTH_014`。
...
- 房间类型：挑战。
- 主要目标：在土块回升途中从移动平台换乘，并在正确高度跳向下方出口。
- 预期洞察：回升平台与周期平台的交点是可预测时机。
...
- 左侧接006；中央土块竖直恢复；下方移动平台水平经过；出口位于下侧安全台。
- 静态地形使用标准`Terrain` Tilemap；镜子放置点必须是安全`StaticSolid`。
- 动态对象完整行程不得夹住角色、侵入出口或形成存活软锁。

## Prefab需求

- SinkingBlock-A：宽3、行程3；MovingPlatform-A：水平5格往返、端点等待0.5秒。
