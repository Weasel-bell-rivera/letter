...
- 房间类型：收束。
- 主要目标：依次完成镜像双承重、土块与移动平台接轨、普通压力板门控。
- 预期洞察：复杂房间应拆成三个可独立恢复的重量状态，而非一次完美同步。
...
- 第一段对称双土块，第二段土块接轨移动平台，第三段单压力板普通门；段间均有静态安全台。
- 静态地形使用标准`Terrain` Tilemap；镜子放置点必须是安全`StaticSolid`。
- 动态对象完整行程不得夹住角色、侵入出口或形成存活软锁。

## Prefab需求

- SinkingBlock-A/B/C；MovingPlatform-A；PressurePlate-A+Door-A；无敌人。
