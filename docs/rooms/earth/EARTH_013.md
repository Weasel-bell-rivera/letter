...
- Unity Scene：`Assets/Scenes/Levels/Earth/Earth_013.unity`（已创建并登记Build Settings）。
- 地图连接：右`EARTH_012`。
...
- 房间类型：可选精通。
- 主要目标：用一次地面镜操控错层双路线，让双方先后压住不同土块并在中央会合。
- 预期洞察：反向输入可通过地形高度差转化为不同的行动顺序。
