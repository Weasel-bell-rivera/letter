...
- Unity Scene：`Assets/Scenes/Levels/Earth/Earth_014.unity`（已创建并登记Build Settings）。
- 地图连接：上`EARTH_010`，下`EARTH_016`；016仍为占位，正式出口待同步。
...
- 房间类型：挑战。
- 主要目标：在下沉土块和移动平台交叉的两层路线中完成两次换乘。
- 预期洞察：动态表面可以组合，但每次只需判断当前支撑面。
...
- 竖向土块穿过两条水平平台轨迹；每个交点旁都有静态等待台。
- 静态地形使用标准`Terrain` Tilemap；镜子放置点必须是安全`StaticSolid`。
- 动态对象完整行程不得夹住角色、侵入出口或形成存活软锁。

## Prefab需求

- SinkingBlock-A：宽3、行程4；MovingPlatform-A/B：上下两层水平往返。
