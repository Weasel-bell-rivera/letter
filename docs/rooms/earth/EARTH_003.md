...
- Unity Scene：`Assets/Scenes/Levels/Earth/Earth_003.unity`（已创建并登记Build Settings）。
- 地图连接：上`EARTH_001`，下`EARTH_004`。
...
- 房间类型：练习。
- 主要目标：先把土块压到底，再离开并利用其缓慢回升抵达上层。
- 预期洞察：卸重不是失败；恢复运动本身可以成为下一步。
...
- 入口在上方，中央土块沉到底后接通下层凹室；离开后从侧台跳回正在上升的土块。
- 静态地形使用标准`Terrain` Tilemap；镜子放置点必须是安全`StaticSolid`。
- 动态对象完整行程不得夹住角色、侵入出口或形成存活软锁。

## Prefab需求

- SinkingBlock-A：位置(0,1)，宽3，行程3，满沉重量1，恢复速度1。
