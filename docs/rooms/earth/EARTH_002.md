...
- Unity Scene：`Assets/Scenes/Levels/Earth/Earth_002.unity`（已创建并登记Build Settings）。
- 地图连接：右`EARTH_001`。
...
- 房间类型：引入。
- 主要目标：踩住一块压沉土块，将自身送到下层出口。
- 预期洞察：土块不仅是按钮，也是可站立的升降地形。
...
- 固定单屏；上表面高度从y=0降至y=-2，土块全行程及落点同时可见。
