...
- Unity Scene：`Assets/Scenes/Levels/Earth/Earth_012.unity`（已创建并登记Build Settings）。
- 地图连接：右`EARTH_011`，左`EARTH_013`。
...
- 房间类型：精通。
- 主要目标：观察敌人往返周期，利用土块高度安全穿越两段墙面。
- 预期洞察：敌人是可预测的移动限制，土块负责改变玩家路线。
...
- 两面竖墙各一只墙行敌人；中央土块提供高低两条安全路线；无门、无尖刺。
- 静态地形使用标准`Terrain` Tilemap；镜子放置点必须是安全`StaticSolid`。
- 动态对象完整行程不得夹住角色、侵入出口或形成存活软锁。

## Prefab需求

- SinkingBlock-A：宽3、行程2；VerticalWallPatrolEnemy-A/B：固定Scene实例。
