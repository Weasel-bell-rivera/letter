...
- Unity Scene：`Assets/Scenes/Levels/Earth/Earth_011.unity`（已创建并登记Build Settings）。
- 地图连接：上`EARTH_007`，左`EARTH_012`，下`EARTH_015`。
- 世界进入顺序与土区解锁条件仍待确定。

## 房间定位

- 房间类型：组合。
- 主要目标：让土块下降并暂时封住墙行敌人的竖直路线，安全通过中央通道。
- 预期洞察：地形变化可以改变敌人的可达路线，但不改变敌人规则。
...
- 中央普通竖墙有固定墙行敌人；相邻土块降到底后以实体边界截断其路径，玩家从下方绕行。
- 静态地形使用标准`Terrain` Tilemap；镜子放置点必须是安全`StaticSolid`。
- 动态对象完整行程不得夹住角色、侵入出口或形成存活软锁。

## Prefab需求

- SinkingBlock-A：宽2、行程2；VerticalWallPatrolEnemy-A：固定Scene实例。
