...
- Unity Scene：`Assets/Scenes/Levels/Earth/Earth_007.unity`（已创建并登记Build Settings）。
- 地图连接：上`EARTH_005`，左`EARTH_008`，右`EARTH_006`，下`EARTH_011`。
...
- 房间类型：节奏/组合。
- 主要目标：完成一个低压力双土块谜题，并看懂通往左、右、下方的三个出口。
- 预期洞察：同一条重量规则可以改变多个方向的通路。
...
- 中央安全大厅；左右土块分别降到支路高度；下方出口无需额外机关，避免支路选择软锁。
- 静态地形使用标准`Terrain` Tilemap；镜子放置点必须是安全`StaticSolid`。
- `Terrain` Tilemap使用`Assets/Tiles/Earth/Earth007/Earth007Terrain.asset`呈现低色差、粗分区的low-poly土地纹理；该视觉替换不改变格子布局、碰撞或`StaticSolid`表面语义。
- 动态对象完整行程不得夹住角色、侵入出口或形成存活软锁。

## Prefab需求

- SinkingBlock-A/B：左右对称、宽3、行程2、满沉重量1。
...
- 重新进入：加载本房全部瞬时对象初始状态。
- `DEFAULT`入口位于左出口Trigger内侧并保持安全间距，直接从本Scene启动时不得立即触发离场。
