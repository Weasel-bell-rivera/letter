...
两门校准为(5.5,7)和(8.5,0)，第二压力板底边贴合Y=-1；第一扇门上方补齐X=5,Y=8～10墙帽。返回入口移到(9.45,-0.08)，右下出口中心对齐X=11.5。相机边界改为X[-12,12]、Y[-2,15]，纵向显式构图偏移为(0,0.56)，不再保留旧房间高度。

| 入口ID | Player Collider中心 |
|---|---|
| `FROM_FIRE_004` | `(9.45,-0.08)` |
| `DEFAULT` | `(-7.5,12.92)` |

| 已实现出口 | 中心 | 目标Scene/入口ID |
|---|---|---|
| `Exit-A to FIRE_004` | `(11.5,0.2)` | `Fire_004/FROM_FIRE_003` |
...
以上是本轮修复前的历史差异；入口和相机的最新修复见文首“2026-08-31 房间几何修复”。旧Builder仍不得用于覆盖当前手工Scene。
