...
补齐FIRE_013返回所需的独立来源入口，位置与DEFAULT一致。地形与投火者未改；“初始在攻击带外、向左安全出口移动，却必须让镜像替身”的设计矛盾仍待决定，不以新增门或危险强制解法。

| 入口ID | Player Collider中心 |
|---|---|
| `DEFAULT` | `(0,-1.08)` |
| `FROM_FIRE_013` | `(0,-1.08)` |

| 已实现出口 | 中心 | 目标Scene/入口ID |
|---|---|---|
| `Exit-A to FIRE_010` | `(-7,-1)` | `Fire_010/DEFAULT` |
