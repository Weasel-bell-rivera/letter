...
第三门后的下方出口增加独立安全落脚区：Terrain底板X=-13～-10,Y=-14，右侧封边X=-9,Y=-13～-11。出口移到(-12,-11.5)，位于下方横向小室中，不嵌入左边界；FROM_FIRE_009移到(-9.95,-12.08)。保留三组永久门、两处检查点、两条岩浆沟和原有存档ID。相机显式偏移(0,0.56)，边界仍为X[-14,15]、Y[-14,14]。

| 入口ID | Player Collider中心 |
|---|---|
| `DEFAULT` | `(2.5,8.92)` |
| `FROM_FIRE_009` | `(-9.95,-12.08)` |
| `FROM_FIRE_007` | `(2.5,8.92)` |

| 已实现出口 | 中心 | 目标Scene/入口ID |
|---|---|---|
| `Exit-A to FIRE_009` | `(-12,-11.5)` | `Fire_009/DEFAULT` |
