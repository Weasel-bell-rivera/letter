...
放镜/DEFAULT观察点改为(-1.5,-1.08)，Door-A移到(-6.5,-1)并将墙帽同步到X=-7,Y=0～5。右侧投火者移到(7.5,-0.5)，由(7,-2)地形格支撑。板后仅保留(5,-2)止挡格，板上方(4,0)形成净高2 units的低顶；镜像停在X=4.6时仍占板，火球从Y=-0.5通过止挡上方。喷发沿用1/1/2s周期，使用已修正为1×4世界范围的共用Prefab。

| 入口ID | Player Collider中心 |
|---|---|
| `DEFAULT` | `(-1.5,-1.08)` |
| `FROM_FIRE_012` | `(-1.5,-1.08)` |

| 已实现出口 | 中心 | 目标Scene/入口ID |
|---|---|---|
| `Exit-A to FIRE_012` | `(-11,-1)` | `Fire_012/FROM_FIRE_011` |
