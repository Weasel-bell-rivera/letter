...
左上入口向房内移到(-6.4,3.92)，解决离返回出口过近。三层地形、低顶、锁存板背板及双投火者配置保持。

| 入口ID | Player Collider中心 |
|---|---|
| `FROM_FIRE_006` | `(3.5,-5.08)` |
| `DEFAULT` | `(-6.4,3.92)` |
| `FROM_FIRE_004` | `(-6.4,3.92)` |

| 已实现出口 | 中心 | 目标Scene/入口ID |
|---|---|---|
| `Exit-To-FIRE006` | `(8.5,-4.8)` | `Fire_006/DEFAULT` |
| `Exit-Back-to-FIRE004` | `(-8.5,4.2)` | `Fire_004/FROM_FIRE_005` |
