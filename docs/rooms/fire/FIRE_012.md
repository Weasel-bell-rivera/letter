...
DEFAULT放镜点改为(0,-1.08)，Plate-A为(8,-1.85)；止挡从X=10高墙改为(9,-2)，低顶为(8,0)。投火者移到(11.5,-0.5)，由(11,-2)格支撑；火线Y=-0.5，镜像停点X=8.6。门仍为(-4.5,-1)，补齐(-5,0)墙帽；相机水平跟随模式不变，使用显式构图偏移(0,0.56)。

| 入口ID | Player Collider中心 |
|---|---|
| `DEFAULT` | `(0,-1.08)` |
| `FROM_FIRE_011` | `(0,-1.08)` |

| 已实现出口 | 中心 | 目标Scene/入口ID |
|---|---|---|
| `Exit-A Return to FIRE_011` | `(9.5,3)` | `Fire_011/FROM_FIRE_012` |
