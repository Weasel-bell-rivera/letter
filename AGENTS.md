...
- 允许agent使用文本补丁、Python、Shell等外部工具增量修改文本序列化的Scene和Prefab，包括Unity当前已加载的Scene；不要求先关闭场景，也不强制通过MCP写入。
- 外部编辑后的Reload和Editor保存遵循`docs/UNITY_MCP_WORKFLOW.md`中的“Scene写入与外部文件同步”；不得覆盖或丢弃用户未保存的编辑。
- 同一Scene同一时刻只允许一个执行者写入；子代理可以并行审查或处理互不重叠的资源，不得并发修改同一场景。
- 通用机制不能写死在具体房间Scene中。
...
- 用户已明确授权agent为当前任务进入Unity Editor的Play Mode，无需再次确认。允许暂停、逐帧推进、恢复运行和退出，以完成运行观察、截图和可视化检查；多轮视觉优化取图可重复执行，不必逐轮申请。
- 此授权同样适用于Unity MCP和Editor UI操作。进入Play Mode与运行Unity Test Framework的PlayMode自动测试是不同操作，不得仅因需要进入Play Mode就套用下文的测试确认要求。
- 本节是本项目关于Play Mode运行与取图的明确规则；其他文档中笼统的“PlayMode需要确认”不应解释为限制本节已授权的操作。
- 操作前仍须检查当前Scene、dirty状态和Prefab Stage，保留用户未保存编辑；不得为进入或退出Play Mode而擅自保存、丢弃、覆盖这些编辑，也不得擅自中断用户正在进行的试玩。
- 此授权不包含PlayMode自动测试、完整EditMode测试、完整项目编译、全量资源重导入、项目复制、整房重建、完整人工试玩或其他需确认的耗时检查；这些操作仍遵循下文原有要求。
- 运行截图只证明对应画面状态，不得将其报告为自动测试或完整试玩通过。
...
- 通过Unity MCP执行的PlayMode自动测试、完整EditMode测试、完整编译、完整资源重新导入和其他耗时操作，仍适用本文件的确认规则；进入、暂停、恢复和退出Play Mode按上文常驻授权执行，无需另行确认。
...
6. 通过代码审查和低成本静态检查验证重置、死亡、重复放置和场景切换等边界情况；可按上文常驻授权进入Play Mode进行运行观察和截图。**非必须：除非用户明确要求，否则不为此自动运行PlayMode自动测试、完整测试或人工试玩。**
7. 汇报修改内容、验证证据以及未验证风险。

房间设计、生成和增量修改还必须遵循：

- 开始前阅读`docs/LEVEL_BUILDING.md`的“房间通用生成与验收规则”，具体阈值与行为引用各系统权威文档，不在房间中另定标准。
- 新建房间逐项检查；增量修改逐项判断影响范围并检查受影响对象及关联关系。交付时记录通过、错误、已批准例外、待验证或不适用及依据；未重验的部分不得声称通过。
- 不以对象数量、引用正确或截图代替解法验证，不将制作错误自行解释为例外。明确错误不得标记为验收通过，未执行的运行验证必须记录风险。
- 该清单不扩大当前任务范围，也不改变上述耗时操作确认、Play Mode常驻授权、未保存编辑保护或增量修改规则。
...
- 功能具有可运行场景或可复用Prefab落点；可按上文常驻授权进入Play Mode进行运行观察和截图。**非必须：除非用户明确要求，否则不运行PlayMode自动测试或人工试玩**，未验证的行为必须记录为运行时风险
- 不破坏现有镜子和分身行为
- 失败和重置路径可用
